from models import ActionSubType
...
def extract_skill_from_action_type(action_type: ActionSubType) -> str:
    skills = []

    # Attended an offline event
    if action_type == ActionSubType.ATTENDED_AN_OFFLINE_EVENT:
        skills.extend(["communication", "community_collaboration"])

    # Business plan
    elif action_type == ActionSubType.BUSINESS_PLAN:
        skills.extend(
            [
                "communication",
                "critical_thinking",
    ...
    # Carry a cloth bag
    elif action_type == ActionSubType.CARRY_A_CLOTH_BAG:
        skills.extend(["problem_solving", "citizenship", "hands_on"])

    # Created a Campaign
    elif action_type == ActionSubType.CREATED_A_CAMPAIGN:
    ...
    # Created solution
    elif action_type == ActionSubType.CREATED_SOLUTION:
        skills.extend(["critical_thinking", "problem_solving", "grit", "hands_on"])

    # Crowdsourced data
    elif action_type == ActionSubType.CROWDSOURCED_DATA:
        skills.extend(
            [
                "communication",
                "applied_empathy",
    ...
    elif action_type == ActionSubType.ENGAGED_PEOPLE_THROUGH_SESSIONS:
        skills.extend(["communication", "applied_empathy", "community_collaboration"])

    # Followed up
    elif action_type == ActionSubType.FOLLOWED_UP:
        skills.extend(["communication", "grit"])

    # Implemented existing solution
    elif action_type == ActionSubType.IMPLEMENTED_EXISTING_SOLUTION:
        skills.extend(["critical_thinking", "hands_on"])

    # Joined a Campaign
    elif action_type == ActionSubType.JOINED_A_CAMPAIGN:
        skills.extend(["community_collaboration"])

    # Non tech prototype
    elif action_type == ActionSubType.NON_TECH_PROTOTYPE:
        skills.extend(["critical_thinking", "entrepreneurial", "hands_on"])

    # Non tech solution
    elif action_type == ActionSubType.NON_TECH_SOLUTION:
        skills.extend(["critical_thinking", "problem_solving", "hands_on"])

    # Old report followup
    elif action_type == ActionSubType.OLD_REPORT_FOLLOWUP:
        skills.extend(["communication", "grit"])

    # Project idea
    elif action_type == ActionSubType.PROJECT_IDEA:
        skills.extend(
            ["communication", "problem_solving", "applied_empathy"]
        )

    # Prototype
    elif action_type == ActionSubType.PROTOTYPE:
        skills.extend(
            [
                "communication",
                "critical_thinking",
    ...
    # Reported issue
    elif action_type == ActionSubType.REPORTED_ISSUE:
    ...
    elif action_type == ActionSubType.SEGREGATE_WASTE_AT_SOURCE:
        skills.extend(["problem_solving", "citizenship", "hands_on"])

    # Shared Public Opinion
    elif action_type == ActionSubType.SHARED_PUBLIC_OPINION:
        skills.extend(["citizenship"])

    # Solved a real world problem
    elif action_type == ActionSubType.SOLVED_A_REAL_WORLD_PROBLEM:
        skills.extend(["problem_solving", "applied_empathy", "citizenship"])

    # Sustainable Lifestyle
    elif action_type == ActionSubType.SUSTAINABLE_LIFESTYLE:
        skills.extend(["critical_thinking", "problem_solving"])

    # Swachhata League Participation 2023
    elif action_type == ActionSubType.SWACHHATA_LEAGUE_PARTICIPATION_2023:
        skills.extend(["problem_solving", "citizenship", "hands_on"])

    # Tech prototype
    elif action_type == ActionSubType.TECH_PROTOTYPE:
        skills.extend(["critical_thinking", "entrepreneurial", "grit", "hands_on"])

    # Tech solution
    elif action_type == ActionSubType.TECH_SOLUTION:
        skills.extend(["critical_thinking", "problem_solving", "hands_on"])

    # Mapping asset or issue
    elif action_type == ActionSubType.MAPPING_ASSET_OR_ISSUE:
